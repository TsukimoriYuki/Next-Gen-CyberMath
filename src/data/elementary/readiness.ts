...
const SUBJECT_REVIEW_DATE = "2026-07-18";
const HUMAN_OWNER_REVIEW = {
  reviewedBy: "user",
...
export const ELEMENTARY_RELEASE_GATES: readonly ElementaryReleaseGate[] = Object.freeze([
  ...
  automatic({ id: "qa-pilot-lessons", area: "lesson-quality", title: "pilot lesson QA", description: "9講座の内容構造と参照を検査します。", requiredForBeta: true, requiredForFormal: true, sourceQa: "qa:elementary:pilot-lessons", source: "scripts/check-elementary-pilot-lessons.ts" }),
  automatic({ id: "qa-pilot-problems", area: "problem-quality", title: "pilot problem QA", description: "72問の形式、正答、解説、学年別漢字を検査します。", requiredForBeta: true, requiredForFormal: true, sourceQa: "qa:elementary:pilot-problems", source: "scripts/check-elementary-pilot-problems.ts" }),
  ...
  automatic({ id: "production-scope-guard", area: "publication", title: "production scope guard", description: "承認済みルートだけが200になり、showcaseなどの除外範囲が404になることを確認します。", requiredForBeta: true, requiredForFormal: true, source: "e2e/elementary-limited-beta-production.spec.ts" }),
  ...
  automatic({ id: "elementary-count", area: "content-inventory", title: "小学生版 72問", description: "正式pilot registryの問題数を確認します。", requiredForBeta: true, requiredForFormal: true, sourceQa: "qa:elementary:inventory", source: "src/lib/elementary-inventory.ts" }),
  automatic({ id: "combined-count", area: "content-inventory", title: "全体 1,420問", description: "高校版と小学生版を分けた合計を確認します。", requiredForBeta: true, requiredForFormal: true, sourceQa: "qa:content-inventory:test", source: "scripts/content-inventory-lib.ts" }),
  ...
  automatic({ id: "limited-pilot-scope", area: "guardian-information", title: "小学3年生全体ではなく9講座のpilot", description: "9講座だけで小学3年生全体への対応を示さないよう明記します。", requiredForBeta: false, requiredForFormal: true, defaultStatus: "warning", source: "src/app/elementary/for-guardians/page.tsx", nextAction: "対象単元を拡張し、curriculum coverageを再評価する。" }),
  ...
  manual({ id: "review-math-content", area: "lesson-quality", title: "算数教材の内容妥当性", description: "等分除・包含除、式、単位、正答の妥当性を人間が確認します。", requiredForBeta: true, requiredForFormal: true, source: "src/data/elementary/lessons/math-division.ts", humanReview: { status: "approved", reviewedAt: SUBJECT_REVIEW_DATE, ...HUMAN_OWNER_REVIEW, note: "ユーザー本人が実画面を確認し、問題なしと判断しました。" } }),
  manual({ id: "review-japanese-content", area: "lesson-quality", title: "国語本文・正答の妥当性", description: "本文根拠、人物、気持ち、選択肢を人間が確認します。", requiredForBeta: true, requiredForFormal: true, source: "src/data/elementary/lessons/japanese-feelings.ts", humanReview: { status: "approved", reviewedAt: SUBJECT_REVIEW_DATE, ...HUMAN_OWNER_REVIEW, note: "ユーザー本人が実画面を確認し、問題なしと判断しました。" } }),
  manual({ id: "review-social-content", area: "lesson-quality", title: "社会教材の推測表現", description: "地図から分かることと推測を人間が確認します。", requiredForBeta: true, requiredForFormal: true, source: "src/data/elementary/lessons/social-map.ts", humanReview: { status: "approved", reviewedAt: SUBJECT_REVIEW_DATE, ...HUMAN_OWNER_REVIEW, note: "ユーザー本人が実画面を確認し、問題なしと判断しました。" } }),
  manual({ id: "review-child-safety", area: "child-safety", title: "子ども向け文言", description: "責める表現、不安、競争、固定的役割がないことを人間が確認します。", requiredForBeta: true, requiredForFormal: true, source: "src/data/elementary", humanReview: { status: "approved", reviewedAt: USER_REVIEW_DATE, ...HUMAN_OWNER_REVIEW, note: "ユーザー本人が子ども向け文言に問題なしと確認しました。" } }),
  manual({ id: "review-guardian-information", area: "guardian-information", title: "保護者向け説明", description: "pilotの範囲、採点、保存、権利、未実装事項を人間が確認します。", requiredForBeta: true, requiredForFormal: true, source: "src/app/elementary/for-guardians/page.tsx", humanReview: { status: "approved", reviewedAt: USER_REVIEW_DATE, ...HUMAN_OWNER_REVIEW, note: "ユーザー本人が保護者向け説明に問題なしと確認しました。" } }),
  manual({ id: "review-asset-rights", area: "visual-assets", title: "画像・図の権利確認", description: "自動QAに加えて公開責任者が権利表示を確認します。", requiredForBeta: true, requiredForFormal: true, source: "src/data/elementary/assets/visual-assets.ts", humanReview: { status: "approved", reviewedAt: USER_REVIEW_DATE, ...HUMAN_OWNER_REVIEW, note: "ユーザー本人が画像・図の権利管理に問題なしと確認しました。" } }),
  manual({ id: "review-release-decision", area: "publication", title: "β公開の最終判断", description: "自動判定だけでpublicationStatusを変更しません。", requiredForBeta: true, requiredForFormal: true, source: "src/data/elementary/index.ts", nextAction: "限定betaの運用結果を確認し、正式公開条件は別工程で判断する。", humanReview: { status: "reviewed", reviewedAt: "2026-07-18", ...HUMAN_OWNER_REVIEW, note: "ユーザー本人が限定beta公開を明示承認しました。正式公開はholdを維持します。" } }),
]);
