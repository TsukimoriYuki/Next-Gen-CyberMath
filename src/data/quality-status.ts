...
export const QA_CHECK_DESCRIPTIONS: QaCheckDescription[] = [
  ...
  },
  {
    ...
    title: "問題数整合チェック",
    description:
      "難度別の合計・単元別の合計・公開問題総数が一致するかを検査します。発展問題用の非公開プールなど、意図的に非公開にしている問題が公開集計に混入していないかも確認します。",
  },
  {
  ...
  },
  {
    ...
    title: "冊子型模試の整合チェック",
    description:
      "共通テスト対策の冊子型模試（70分・100点・56マーク）について、配点合計・スロット参照・採点シミュレーションが仕様通りかを検査します。",
  },
  {
...
export const QUALITY_CHANGELOG: { date: string; items: string[] }[] = [
  {
    ...
    items: [
      ...
      "共通テスト対策から、模試後に戻るべき関連する数学講座へ進める復習導線を追加しました。",
      "公開UIのブランド表記と未完成感の強い文言を見直し、サービス名の表記を統一しました。",
    ],
  },
  {
    date: "2026-07-02",
    items: [
      "共通テスト数学I・数学A オリジナル模試 第2回を追加しました。PDF冊子を正本として表示し、採点・解説データだけを構造化しています。",
  ...
  },
  {
    ...
    items: [
      ...
      "発展問題用の非公開プールが、単元一覧・問題数の公開集計に混入していた問題を修正しました。",
      "単元名の表記ゆれ（例:「二次関数」表記の不統一）を解消し、単元一覧の重複表示を解消しました。",
      "共通テスト対策で、診断前に仮スコアが表示されていた古いコンポーネントを削除しました。",
