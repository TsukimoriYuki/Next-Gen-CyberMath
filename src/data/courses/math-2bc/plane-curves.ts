...
{
kind: "concept",
title: "焦点を持つ意味：反射の法則と定義条件",
body: "楕円・双曲線の焦点は、単なる計算上の点ではなく、曲線の本質的な定義に関わります。\n\n楕円の定義：平面上の2点 $F_1,F_2$ からの距離の和が一定の点の集合。\n\n$|PF_1|+|PF_2|=2a$（$2a$ は定数）\n\n双曲線の定義：平面上の2点 $F_1,F_2$ からの距離の差の絶対値が一定の点の集合。\n\n$||PF_1|-|PF_2||=2a$（$2a$ は定数）\n\n楕円の反射の法則：楕円上の点で反射した光は、一方の焦点から出ると必ず他方の焦点に集まります。これは楕円ミラーの原理であり、医療機器（体外衝撃波砕石術）にも応用されています。\n\n受験問題での活用：図形の問題で「2点からの距離の和（または差）が一定」という条件が出たら、楕円（または双曲線）の定義そのものです。標準形に直さなくても、定義から焦点を特定できます。",
},
{
kind: "commonMistake",
