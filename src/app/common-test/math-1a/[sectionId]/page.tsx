...
export async function generateMetadata({
  ...
  const title = `数学IA 第${section.number}問 — ${section.title}`;
  const description = `共通テスト 数学IA 第${section.number}問「${section.title}」の大問別ドリル演習。`;
  const canonical = `/common-test/math-1a/${sectionId}`;
  return {
    title,
    description,
    alternates: {
...
export default async function Math1ASectionPage({
  params,
}: {
  params: Promise<{ sectionId: string }>;
}) {
  ...
  const questions = getCommonTestDrillsBySection("math-1a", sectionId);
